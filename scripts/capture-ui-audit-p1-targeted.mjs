...
async function ensureLoaded(page) {
  ...
  try {
    ...
    // Allow pages with polling/realtime traffic to continue.
  }
}
...
async function login(page, email, password) {
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      // Continue to attempt submit if hydration marker text lags.
    }
...
async function main() {
  ...
  for (const viewport of VIEWPORTS) {
    for (const target of TARGETS) {
      ...
      const filename = `${target.role}__${routeSlug(target.route)}__${viewport.label}__loaded.png`;
  ...
  const manifestPath = path.join(OUT_DIR, "manifest-p1-targeted.json");
