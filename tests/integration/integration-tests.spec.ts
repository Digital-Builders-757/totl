...
// Helper functions
async function registerUser(page: Page, userType: 'talent' | 'client', userData: { email: string; password: string; firstName: string; lastName: string; phone: string; companyName?: string }) {
...
async function createGig(page: Page, gigData: { title: string; description: string; category: string; location: string; budget: string; duration: string; startDate: string; endDate: string }) {
...
test.describe('Error Handling and Edge Cases', () => {
  test('Network error handling', async ({ page }) => {
    ...
    await expect(page.locator('text=Network error') || page.locator('text=Failed to load')).toBeVisible();
  });
