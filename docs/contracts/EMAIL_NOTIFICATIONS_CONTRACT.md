...
| `application-accepted` | `app/api/client/applications/accept/route.ts` `POST()` | API route | Signed-in (client) | `generateApplicationAcceptedEmail({ name, gigTitle, clientName, dashboardUrl })` | `absoluteUrl("/talent/dashboard")` | `tests/api/email-routes.spec.ts` (route exists; internal email routes guarded) |
| `booking-confirmed` | `app/api/client/applications/accept/route.ts` `POST()` | API route | Signed-in (client) | `generateBookingConfirmedEmail({ ... })` | `absoluteUrl("/talent/dashboard")` | `tests/api/email-routes.spec.ts` (route exists; internal email routes guarded) |
