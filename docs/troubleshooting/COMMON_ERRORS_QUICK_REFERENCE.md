...
- **User-facing toasts or alerts show raw Postgres / RLS / `fetch` errors:** Hurts trust and reads “broken app.” **Fix:** Map failures with **`userSafeMessage`** from `@/lib/errors/user-safe-message` (or fixed copy from server actions); log full detail with **`logger`** / **`logActionFailure`** from `@/lib/errors/log-action-failure`. **Prevention:** Do not pass **`error.message`** from Supabase or API JSON straight into UI; use route **`error.tsx`** + calm copy for thrown segment errors (see `docs/TOTL_ERROR_EXPERIENCE_AND_LOGGING_HARDENING_WORK_ORDER_2026.md`). Short, already-safe server strings are passed through after heuristics; very long or SQL/stack-like text still maps to the generic fallback.
- **`userSafeMessage` pass-through must not look like SQL or engine copy:** The final branch only passes through **short** strings; **`messageLooksInternalOrSqlLike`** in `user-safe-message.ts` drops anything with **SQL-looking** DML (e.g. `UPDATE`/`DELETE` as **statement-shaped** tokens, not English **to update** / **to delete**), **`SELECT` / `INSERT` / `TRUNCATE`** substrings, **`MERGE INTO`**, stack frames, length over 200, or Postgres-y fragments (`detail:`, `hint:`, `syntax error`, `relation "`, etc.). **Prevention:** Curated “Failed to update …” style copy is OK; for new patterns add **Vitest** in `lib/errors/user-safe-message.test.ts` if the guard misfires.
- **Legacy server actions still return `{ error: string }`:** At the component boundary, wrap with **`userSafeMessageFromActionError(result.error, "Your fallback copy")`** so forwarded DB/transport text never reaches the toast unchanged.
- **Production API routes echo `error.message` in JSON 500 responses:** Leaks internals to browsers and support tickets. **Fix:** Return a fixed, generic **`error`** string (or stable **`code`**) and log with **`logger.error`** + route name / ids. **Exception:** Deliberate **dev-only** routes (e.g. under **`/api/dev/`**) may stay verbose when product agrees.
- **Route has `loading.tsx` but no `error.tsx`:** Users get a spinner but weak recovery when a server throw occurs. **Fix:** Add route-level `error.tsx` (or segment/root `app/error.tsx`) using a shared fallback component with retry + digest-aware logging. **Prevention:** For high-traffic routes, treat loading/error boundaries as a pair.
- **Public page uses `createSupabaseServer()` (cookies) for read-only content and stays `force-dynamic`:** This blocks cacheability and inflates TTFB. **Fix:** Use a cookie-free server Supabase client for anonymous/public reads and pair with route `revalidate` when data can be stale-tolerant. **Prevention:** Reserve `createSupabaseServer()` for session-aware routes.
- **Schema/type checks fail with Supabase CLI `Unauthorized` while local `supabase projects list` works:** Older pinned `npx supabase@2.34.3` commands can fail auth with newer Supabase auth flows. **Fix:** Run verification/type scripts with a current CLI pin (`2.95.4` in this repo) and clear forced `SUPABASE_ACCESS_TOKEN` overrides when they are stale. **Prevention:** Keep CLI pin in schema/type scripts aligned with currently working `supabase` auth behavior.
- **`app/global-error.tsx` looks unstyled (no theme colors):** The global error boundary replaces the **root** layout, so it does not inherit **`app/layout.tsx`**’s CSS. **Fix:** Add **`import "./globals.css"`** at the top of **`app/global-error.tsx`** (same as root layout). **Prevention:** Any new root-level error UI must load global styles explicitly.
...
- **`supabase migration new ...` hangs and leaves empty migration files in `supabase/migrations/`:**
  - **Symptom:** CLI appears stuck; multiple timestamped migration files are created with `0` bytes.
  - **Fix:** Keep only the intended migration file, delete empty duplicates, then re-run checks (`schema:verify:comprehensive`, `types:check`, `build`, `lint`) before commit.
  - **Prevention:** If CLI hangs repeatedly on Windows shells, stop the process promptly and verify migration file sizes before continuing.
...
- **Radix Dialog / drawer: full-screen dim overlay but no visible panel (“black screen”) or actions off-screen:** Causes include (1) **`.panel-frosted`** sets **`position: relative`**, which can override a plain **`fixed`** on the same node so the panel does not stack with the backdrop; (2) **`DropdownMenu`** + **`Dialog`** opening in the same tick can confuse focus/stacking on mobile; (3) centered **`translate-y-[-50%]`** modals taller than the viewport clip the panel above/below the fold so users only see the dim **`bg-black/80`** overlay.
  - **Fix (shipped):** **`components/ui/dialog.tsx`** uses **`!fixed`**, overlay **`z-[100]`**, content **`z-[101]`**, **`w-[calc(100vw-2rem)]`**, and **`max-h-[min(90dvh,calc(100dvh-2rem))] overflow-y-auto`** so the modal body scrolls inside the viewport. Full-height drawers / bottom sheets add **`!max-h-none`** (and explicit **`h-[…]`**) so they are not capped by that max-height. Open nested dialogs from menu actions with **`queueMicrotask(() => setOpen(true))`** after **`setSelected…`** so the dropdown unmounts first (see **`admin-client-applications-client`** / **`admin-applications-client`**).
  - **Prevention:** Keep **`panel-frosted`** on dialog panels but rely on shared **`DialogContent`** defaults; for custom full-viewport **`DialogContent`**, always include **`!max-h-none`** when setting a fixed height.
