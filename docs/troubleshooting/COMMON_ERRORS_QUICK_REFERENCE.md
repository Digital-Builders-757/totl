...
- **`userSafeMessage` pass-through must not look like SQL or engine copy:** The final branch only passes through **short** strings; **`messageLooksInternalOrSqlLike`** in `user-safe-message.ts` drops anything with statement-shaped keywords (e.g. `update `, `delete `, `merge `), stack frames, length over 200, or Postgres-y fragments (`detail:`, `hint:`, `syntax error`, `relation "`, etc.). **Prevention:** Curated hand-written error strings for pass-through should avoid substrings that look like SQL; add **Vitest** cases in `lib/errors/user-safe-message.test.ts` when introducing new “safe to show as-is” patterns.
