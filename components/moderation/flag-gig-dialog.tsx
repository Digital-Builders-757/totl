...
import { flagGigAction } from "@/lib/actions/moderation-actions";

const DEFAULT_REASONS = [
  "Scam or payment concerns",
...
type FlagGigDialogProps = {
  gigId: string;
  disabled?: boolean;
};

export function FlagGigDialog({ gigId, disabled }: FlagGigDialogProps) {
  ...
  const handleSubmit = () => {
    if (!selectedReason?.trim()) {
      toast({
        ...
        description: "Please select or provide a reason for flagging this gig.",
        variant: "destructive",
      });
      return;
    }
    ...
    startTransition(async () => {
      const result = await flagGigAction({
        gigId,
      ...
      toast({
        ...
        description: "Thank you for helping us keep the marketplace safe.",
      });
      setReason("");
      setDetails("");
      setCustomReason("");
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Let the TOTL moderation team know if something looks suspicious or violates our community
            guidelines.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <Label>Why are you reporting this gig?</Label>
